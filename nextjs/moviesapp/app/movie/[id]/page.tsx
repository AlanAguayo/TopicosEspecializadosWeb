...
async function getMovieDetails(id: MovieCardInterface["id"]) {
  const res = await fetch(
    `https://api.themoviedb.org/3/movie/${id}?api_key=${process.env.API_KEY}&language=es-MX`
  );
  return res.json();
}
...
async function getMovieCasts(id: MovieCardInterface["id"]) {
  const res = await fetch(
    `https://api.themoviedb.org/3/movie/${id}/credits?api_key=${process.env.API_KEY}&language=es-MX`
  );
  return res.json();
}
